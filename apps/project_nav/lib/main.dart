...
/// Global reference so theme-changed messages can reach the provider
  ...
  MessageHelper.listen((type, payload) async {
    if (type == 'init-context') {
      try {
        ...
        runApp(ProjectNavApp(initialThemeMode: themeMode));
...
class ProjectNavApp extends StatelessWidget {
  final String initialThemeMode;

  const ProjectNavApp({super.key, required this.initialThemeMode});

  @override
  Widget build(BuildContext context) {
    final themeProvider = ThemeProvider(
      initialMode: initialThemeMode == 'dark' ? ThemeMode.dark : ThemeMode.light,
    );
